...
dta <- data.frame(age = c(1, 1, 2, 2, 3, 3), 
                ...
                col.chapeau = c("vert", "jaune", "bleu", "jaune", "bleu", "vert"),
...
c1 <- sqrt(nrow(dta))*0.8
c2 <- sqrt(20)/2
Gvar <- partition_variables(dta)

res1 <- SMCA::MCA(dta, n = 8)
res2 <- SMCA::SMCA(dta, c1 = c1, c2 = c2, n = 8)
```
...
```{r, echo=F}
p1 <- plot_SMCA(res1, choix = "ind", aff.noms = T, habillage = 1) + theme(legend.position = "none")+ ggtitle("MCA")
p2 <- plot_SMCA(res2, choix = "ind", aff.noms = T, habillage = 1) + theme(legend.position = "none")+ ggtitle("SMCA")
...
p <- ggarrange(p1, p2, common.legend = T, legend = "bottom", heights = 10, widths = 10)
...
p1 <- plot_SMCA(res1, choix = "var", aff.noms = T) + theme(legend.position = "none")+ ggtitle("MCA")
p2 <- plot_SMCA(res2, choix = "var", aff.noms = T) + theme(legend.position = "none")+ ggtitle("SMCA")
p <- ggarrange(p1, p2, common.legend = T, legend = "bottom")
