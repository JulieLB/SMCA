...
```{r}
plot_SMCA(res1, choix = "ind", aff.noms = T, habillage = 1)
plot_SMCA(res1, choix = "mod", aff.noms = T)
plot_SMCA(res1, choix = "var", aff.noms = T, title.precision = "(MCA)")
...
```{r}
plot_SMCA(res2, choix = "ind", aff.noms = T, habillage = 1)
plot_SMCA(res2, choix = "mod", aff.noms = T)
plot_SMCA(res2, choix = "var", aff.noms = T)
...
for (i in 1:length(c1)) {
  for (j in 1:length(c2)) {
    ...
    p[[k]] <- plot_SMCA(res, choix = "mod", aff.noms = T, title.precision = paste("(c1 =", c1[i], ", c2 =", c2[j], ")", sep = " "))
    k <- k + 1
  }
}
