...
csvd <- function (X,
  ...
  res1 <- PI(X, c1, c2, P0 = P0[, 1], Q0 = Q0[, 1], eps.pi = eps.pi, itermax.pi = itermax.pi, eps.pocs = eps.pocs, itermax.pocs = itermax.pocs, Gcol = Gcol, Grow = Grow)
  P[, 1] <- res1$p
  Q[, 1] <- res1$q
  ...
  if (R!=1) {
    for (i in 2:R) {
      res2 <- PI(X, c1, c2, P0[,i], Q0[,i], P[,1:i-1], Q[,1:i-1], eps.pi = eps.pi, itermax.pi = itermax.pi, eps.pocs = eps.pocs, itermax.pocs = itermax.pocs, Gcol = Gcol, Grow = Grow)
      P[,i] <- res2$p
      Q[,i] <- res2$q
